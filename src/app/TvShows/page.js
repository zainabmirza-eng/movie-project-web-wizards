            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 justify-center gap-5 p-8 bg-gradient-to-r from-[#000814] from-10% via-[#001d3d] via-30% to-[#000814] to-90% ... text-white'>
                 {TvShows.map((show) => (
                    <div  key={show.id} className="items-center rounded-lg shadow-md border-white p-4 hover:shadow-cyan-900 hover:border-cyan-800 border-2 border-transparent hover:font-bold">
                        <img src={`https://image.tmdb.org/t/p/w500${show.poster_path}`} alt={show.name}  className="rounded-lg shadow-2xl w-full" />
                        <h2 className="text-lg font-semibold p-4 text-center" >{show.name}</h2>
                    </div>
                ))}
            </div>
        </div>
