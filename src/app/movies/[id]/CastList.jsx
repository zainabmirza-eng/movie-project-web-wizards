                ...
                <div className="flex flex-wrap gap-2">
                    {uniqueCasts.map((item, i) => (
                        <Link key={i} href={`/actors/${item.id}`} passHref>
                            <p className="text-gray-900 bg-gradient-to-r from-blue-200 to-gray-200 hover:bg-gradient-to-l hover:from-blue-200 hover:to-gray-200 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-teal-700 font-medium rounded-lg text-sm px-5 py-1.5 text-center mb-2">
                                {item.name}
                            </p>
                        </Link>
                    ))}
