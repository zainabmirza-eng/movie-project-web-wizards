    ...
    {movies.results.map((item) => (
      <Link key={item.id} href={`./movies/${item.id}`}>
        <div className="group relative flex flex-col items-center">
          <div className="relative w-72 h-120 overflow-hidden rounded-xl shadow-xl">
            <img
              alt="poster"
              className="w-full h-3/4 object-cover transition duration-500 transform hover:scale-110"
              src={`https://image.tmdb.org/t/p/original/${item.poster_path}`}
            />
            <div className="absolute inset-0 bg-[#1b263b] bg-opacity-80 text-white font-bold opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center">
          ...
          {/* Movie Information */}
          <div className="absolute group-hover:opacity-0 transition-opacity duration-300 bottom-0 rounded-lg left-0 w-full h-1/4 bg-[#1b263b] bg-opacity-80 text-white font-bold flex flex-col items-center justify-center">
            <p className="text-lg font-semibold uppercase text-center">{item.original_title}</p>
          </div>
        </div>
      </Link>
    ))}
